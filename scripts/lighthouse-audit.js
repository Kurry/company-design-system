...
class PerformanceAuditor {
  ...
  async runAudit() {
    ...
    try {
      ...
      try {
        ...
        const { lhr, report } = await lighthouse(`file://${this.previewPath}`, {
  ...
  analyzeResults(lhr) {
    ...
    Object.entries(scores).forEach(([category, score]) => {
      ...
      console.log(
        `${icon} ${this.capitalize(category.replace(/([A-Z])/g, ' $1'))}: ${chalk[color](score)}/100`
      );
    });
    ...
    if (audits['render-blocking-resources']) {
      ...
      console.log(
        `Render Blocking: ${rbr.score === 1 ? chalk.green('✅ Optimized') : chalk.yellow('⚠️ ' + rbr.displayValue)}`
      );
    }
    ...
    if (audits['unused-css-rules']) {
      ...
      if (unusedCSS.details && unusedCSS.details.items.length > 0) {
        const wastedBytes = unusedCSS.details.items.reduce(
          (total, item) => total + item.wastedBytes,
          0
        );
        console.log(
          `Unused CSS: ${chalk.yellow('⚠️ ' + this.formatBytes(wastedBytes) + ' could be removed')}`
        );
      } else {
    ...
    const failedScores = Object.entries(scores).filter(
      ([category, score]) => score < this.scoreThresholds[category]
    );
    ...
    if (failedScores.length > 0) {
      ...
      failedScores.forEach(([category, score]) => {
        console.log(
          `   • ${this.capitalize(category.replace(/([A-Z])/g, ' $1'))}: ${score}/100 (threshold: ${this.scoreThresholds[category]})`
        );
      });
